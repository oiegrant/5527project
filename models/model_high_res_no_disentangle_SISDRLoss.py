...
def save_samples(model, num_samples, total_samples, epoch):
    ...
    for index in random_indices:
        ...
        noisy_audio = audio_sample + 0.1 * torch.rand(audio_sample.shape[0])
        cleaned_audio = model(video_sample.unsqueeze(dim=0).to(device), noisy_audio.unsqueeze(dim=0).to(device))
        write_sample(noisy_audio, audio_sample, cleaned_audio.squeeze(dim=0).to('cpu').detach(), epoch, index)

def write_sample(noisy_audio_tensor, clean_audio_tensor, cleaned_audio_tensor, epoch, sample):
  root = "sample_data"
  batch_dir = f'epoch{epoch}'
  sample_dir = f'sample{sample}'
  path = os.path.join(root, batch_dir, sample_dir)
...
loss_filename = 'loss_high_res_no_disentangle_10speaker_batch1.txt'
model_weights_path = 'model_weights_high_res_no_disentangle_10speaker_batch1.pth'
...
k = 0
model.eval()
save_samples(model, 4, num_samples, k)
model.train()
...
for i in range(epochs):
    for j in range(num_samples // batch_size):
        ...
        for k in range(start, end):
            ...
            #audio_augmentation = hdf5_file.read_audio_tensor(audio_agumentation_index)
