...
class AVSpeechSeparator(nn.Module):
    def __init__(self, audio_samples, frames, image_height, image_width, patch_size, d_model, depth, heads, mlp_dim, dropout, n_fft, hop_length, win_length):
        ...
        self.decoder = nn.TransformerEncoder(
            ...
            num_layers=depth)
        spectrogram_dim = self.time_domain * self.freq_domain * 2
        complex_head_dim = complex_head_dim = spectrogram_dim + (75 * self.freq_domain * 2)
        self.mask_generation_network = MLPComplexHead(complex_head_dim, mlp_dim, spectrogram_dim, self.freq_domain)
...
def train_model(model, criterion, optimizer, scheduler, visual_stream, audio_stream, target):
    optimizer.zero_grad()
    estimate_spectrogram = model(visual_stream, audio_stream)
    target_spectrogram = torch.stft(target, n_fft=model.n_fft, hop_length=model.hop_length, win_length=model.win_length, return_complex=False)
    #loss = criterion(target, estimate)
    loss = criterion(target_spectrogram, estimate_spectrogram)
    loss.backward()
    optimizer.step()
...
def save_samples(model, num_samples, total_samples, batch, epoch):
    ...
    for index in random_indices:
        ...
        cleaned_spectrogram = model(video_sample.unsqueeze(dim=0).to(device), noisy_audio.unsqueeze(dim=0).to(device))
        cleaned_spectrogram = torch.view_as_complex(cleaned_spectrogram)
        cleaned_audio = torch.istft(cleaned_spectrogram, n_fft=model.n_fft, hop_length=model.hop_length, win_length=model.win_length, length = model.audio_samples, return_complex=False)
...
def write_sample(noisy_audio_tensor, clean_audio_tensor, cleaned_audio_tensor, batch, sample, epoch):
  root = "sample_data_no_disentangle_L2Loss_Scheduler_500steps_2SpeakerRMSNormalLossSpectrogram"
...
#hdf5_filename = '../../kosi0036/Dataset/train_data_high_res.h5'
hdf5_file = HDF5(hdf5_filename)
loss_filename = 'loss_high_res_no_disentangle_L2Loss_Scheduler_2SpeakerRMSNormal_75VisualTokensLossSpectrogram.txt'
model_weights_path = 'model_weights_high_res_no_disentangle_L2Loss_Scheduler_2SpeakerRMSNormal_75VisualTokensLossSpectrogram.pth'
...
# visual_tensor = torch.randn(batch_size, frames, rgb_channels, height, width)
# audio_tensor = torch.randn(batch_size, num_audio_samples)
# output = model(visual_tensor.to(device), audio_tensor.to(device))
