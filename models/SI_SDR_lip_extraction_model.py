...
class AVSpeechSeparator(nn.Module):
    def __init__(self, audio_samples, frames, image_height, image_width, patch_size, d_model, depth, heads, mlp_dim, dropout, n_fft, hop_length, win_length):
        ...
        self.decoder = nn.TransformerEncoder(
            ...
            num_layers=depth)
        complex_head_dim = self.time_domain * self.freq_domain * 2
        self.mask_generation_network = MLPComplexHead(complex_head_dim, mlp_dim, complex_head_dim, self.freq_domain)
...
def train_model(model, criterion, optimizer, visual_stream, audio_stream, target):
    optimizer.zero_grad()
...
def create_lip_extraction_protocol():
    fa = FaceAlignment(LandmarksType.TWO_D, device='cuda')
    def lip_extraction_protocol(video_tensor):
        lip_cropped_video_tensor = []

        for val in video_tensor[:]:
            image = val.permute(1, 2, 0).numpy().astype(np.uint8)
            landmarks = fa.get_landmarks(image)
            if landmarks is None:
                raise Exception('Lip extraction failed to detect lips!')

            lips_points = landmarks[0][48:68]  
            min_y, min_x = map(int, lips_points[0])
            ...
            x_length = int((max_x - min_x)*0.5)
            y_length = int((max_y - min_y) * 0.5)

            val = val[:, max(0, min_x - x_length):min(200, max_x + x_length), max(0, min_y - y_length):min(200, max_y + y_length)]
            resized_tensor = F.interpolate(val.unsqueeze(0), size=(32, 64), mode='bilinear', align_corners=False)
            resized_tensor = resized_tensor.squeeze(0)
            lip_cropped_video_tensor.append(resized_tensor)
        video_tensor = torch.stack(lip_cropped_video_tensor)
        return video_tensor
    
    return lip_extraction_protocol
...
lip_extraction_protocol = create_lip_extraction_protocol()
...
hdf5_filename = '../Dataset/train_data.h5'
...
for i in range(epochs):
    for j in range(num_samples // batch_size):
        ...
        for k in range(start, end):
            ...
            try:
                video_sample = lip_extraction_protocol(video_sample)
            except:
                lip_extraction_failure = True
                break
            #video_sample = video_sample[::3, :, :, :]
            audio_agumentation_index = get_audio_agumentation_index(start, end, num_samples)
            audio_agumentation = hdf5_file.read_audio_tensor(audio_agumentation_index)
            video_samples.append(video_sample)
            audio_samples.append(audio_sample)
            audio_agumentations.append(audio_agumentation)
        if lip_extraction_failure:
            continue
        video_samples = torch.stack(video_samples)
        audio_samples = torch.stack(audio_samples)
        audio_agumentations = torch.stack(audio_agumentations)
        noisy_audio_samples = audio_samples + audio_agumentations
        ...
        print_cuda_memory_usage()
        print('Done!')
        loss = train_model(model, criterion, optimizer, video_samples, noisy_audio_samples, audio_samples)
