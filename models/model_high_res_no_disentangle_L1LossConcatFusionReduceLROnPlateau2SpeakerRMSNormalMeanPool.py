...
class AVSpeechSeparator(nn.Module):
    def __init__(self, audio_samples, frames, image_height, image_width, patch_size, d_model, depth, heads, mlp_dim, dropout, n_fft, hop_length, win_length):
        ...
        self.decoder = nn.TransformerEncoder(
            ...
            num_layers=depth)
        complex_head_dim_in = 4 * self.freq_domain 
        complex_head_dim_out = complex_head_dim = self.time_domain * self.freq_domain * 2
        self.mask_generation_network = MLPComplexHead(complex_head_dim_in, mlp_dim, complex_head_dim_out, self.freq_domain)
    ...
    def forward(self, visual_stream, acoustical_stream):
        ...
        acoustical_emb = torch.mean(acoustical_emb, dim = 1).unsqueeze(dim=1)
...
def write_sample(noisy_audio_tensor, clean_audio_tensor, cleaned_audio_tensor, batch, sample, epoch):
  root = "sample_data_no_disentangle_L1Loss_Scheduler_500steps_2SpeakerRMSNormalMeanPool"
...
loss_filename = 'loss_high_res_no_disentangle_L1Loss_Scheduler_2SpeakerRMSNormalMeanPool.txt'
model_weights_path = 'model_weights_high_res_no_disentangle_L1Loss_Scheduler_2SpeakerRMSNormalMeanPool.pth'
